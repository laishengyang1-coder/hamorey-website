...
export function StoreFinder() {
  ...
  const provinces = useMemo(() => getProvinces(allStores), []);
  const cities = useMemo(
    () => (province ? getCitiesByProvince(allStores, province) : []),
    [province],
  );

  const filteredStores = useMemo(() => {
    let result: StoreConfig[] = [...allStores];
              ...
              <ScrollReveal key={store.id} delay={index * 100}>
                <Card hover padding="md" className="h-full flex flex-col">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <Badge variant="brand">{AUTH_LEVEL_LABELS[store.auth_level]}</Badge>
                  </div>
                  <div className="flex flex-col gap-2 text-sm text-content-secondary flex-1">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1">
                      {getStoreProducts(store).map((product) => (
