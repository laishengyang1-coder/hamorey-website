...
export default function AddressPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { const res = await apiRequest<{ items: Address[] }>('/province/addresses'); setData(res.items); }
  ...
  const handleSave = async () => {
    ...
    try {
      if (selected) {
        await apiRequest(`/province/addresses/${selected.id}`, { method: 'PUT', body: JSON.stringify(form) });
      } else {
        await apiRequest('/province/addresses', { method: 'POST', body: JSON.stringify(form) });
      }
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try { await apiRequest(`/province/addresses/${deleteId}`, { method: 'DELETE' }); setDeleteId(null); fetchData(); }
