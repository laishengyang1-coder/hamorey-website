...
export default function RewardsPage() {
    ...
    async function init() {
      ...
      try {
        const [r, a] = await Promise.all([
          apiRequest<{ items: Reward[] }>('/province/rewards'),
          apiRequest<{ items: Address[] }>('/province/addresses'),
        ]);
  ...
  const handleExchange = async () => {
    ...
    setSubmitting(true);
    try {
      await apiRequest('/province/redemptions', {
        method: 'POST',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {rewards.map((r) => (
          <div key={r.id} className="bg-white rounded-xl border border-gray-100 p-4 hover:shadow-sm transition-shadow">
            <div className="h-32 bg-gray-50 rounded-lg mb-3 flex items-center justify-center text-gray-300 text-sm">
              {r.cover_file_key ? '📦' : '暂无图片'}
            </div>
