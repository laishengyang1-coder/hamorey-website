...
Page({
  ...
  async loadDashboard() {
    ...
    try {
      const [summary, storeRank, productRank, pointsRank] = await Promise.all([
        ...
        api.get('/province/dashboard', { type: 'national-points-ranking' }, { loading: false }),
      ]);
      ...
      if (pointsRank.ok) this.setData({ pointsRanking: pointsRank.data || [] });
    } catch (e) {
      // 单点失败不阻塞整体展示
    } finally {
