...
Page({
  data: {
    ...
    cart: {}, cartTotal: 0, cartCount: 0, showCart: false, myPoints: 0
  },
  onShow() {
    const app = getApp();
  ...
  updateCart() {
    ...
    const cart = {};
    rewards.forEach(r => {
      if (r.qty > 0) { cart[r.id] = r.qty; cartTotal += r.qty * (r.points_required || 0); cartCount++; }
    });
  ...
  goCheckout() {
    ...
    const items = rewards.filter(r => r.qty > 0).map(r => ({
      id: r.id, name: r.name, image: r.coverPath || '', points: r.points_required || 0, qty: r.qty
    }));
